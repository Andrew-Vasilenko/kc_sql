/* 5. Написать скрипт вывода клиентов мужского пола старше 30 лет. */

USE KCDB
GO

DECLARE @CURRENT_DATE DATE = GETDATE()
DECLARE @30YRS_BACK DATE = DateAdd(YY, -30, @CURRENT_DATE)
PRINT @30YRS_BACK
...
WHERE ID_FIELD = 4 AND CONVERT(DATE, FIELD) < @30YRS_BACK)
...
BEGIN
	...
	SET @CLIENT_ID = (SELECT MIN(ID_CLIENT)
	FROM CLIENTS_ADDFL_STRING
	WHERE (ID_FIELD = 4 AND CONVERT(DATE, FIELD) < @30YRS_BACK) AND (ID_CLIENT > @CLIENT_ID))
END
